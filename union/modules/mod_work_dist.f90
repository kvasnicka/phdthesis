...
!General comments
!
!Value function is a multi-dimensional array (5 dimensions in this case if thenumber of
!countries is two).
!To distribute work between images (processes), we essentially fold all of these elements
...
!Type for sharing policy function between images (this is slightly different because we don't
!need to share this between iterations)
type shared_data_pol_type
    real(dp), dimension(:,:), allocatable :: x_pol_img !policy function on an image
    real(dp), dimension(:,:), allocatable :: x_pol_all !for policy function for all grid points
    real(dp), dimension(:,:,:,:,:,:), allocatable :: x_pol_all_unf !same as x_pol_all but 'unfolded'. So instead of
    !1 index for cycling over the grid points we have 5 indices. This is used only in CTS algorithm for interpolation.
end type shared_data_pol_type
