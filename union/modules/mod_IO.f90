...
!And create folder pars
shell_cmd = 'mkdir -p results/'//trim(folder_name)//'/pars' !-p creates the directory even if directory results doesn't exist
call system(shell_cmd)
...
subroutine GetReal(variable_name,file_path,parameter_value,default_value)
...
select case(variable_found)
        case(0)
                write(*,*) 'Warning: variable ',variable_name, ' not found in file ', trim(file_path), '.'
                write(*,*) 'Using default value = ',default_value
...
subroutine GetInt(variable_name,file_path,parameter_value,default_value)
        character*(*), intent(in) :: variable_name, file_path
...
select case(variable_found)
        ...
        case(1)
                !everything fine
        case default
                write(*,*) 'Error: variable ',variable_name, ' occurs multiple times in file ', trim(file_path), '.'
                error stop
end select
...
subroutine GetLogical(variable_name,file_path,parameter_value,default_value)
        implicit none
