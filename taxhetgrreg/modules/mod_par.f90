...
type par
        ...
        real(dp) :: rr_delta
        logical :: GD_use_FS
        real(dp) :: GD_reg_lambda
        integer :: GD_max_iter
        real(dp) :: GD_alpha
        real(dp) :: GD_stop_crit

        integer :: GD_MB_iter
        integer, dimension(1,N_MB_th) :: GD_MB_threshold
        real(dp), dimension(1,N_MB_th) :: GD_MB_share

        logical :: gen_mod_V_reg !(GAF stands for Generate approximating functions)
        integer :: GAF_FS
        integer, dimension(1,2) :: GAF_basis !the column dimension is the number of approximated functions
        integer, dimension(2,4) :: GAF_deg !first dimension corresponds to number of underlying states
        integer, dimension(2,4) :: GAF_inter_deg !as above
...
subroutine read_parameters(parameter_file,parameters)
        ...
        call GetReal('disc_share_fit',file_path,parameters%disc_share_fit)
        ...
        call GetReal('perm_eps',file_path,parameters%perm_eps)
        call GetReal('disc_share',file_path,parameters%disc_share)
        call GetInt('T_sim_max',file_path,parameters%T_sim_max)
        call GetInt('reg_info',file_path,parameters%reg_info)
        call GetInt('grids_type',file_path,parameters%grids_type)
        call GetInt('V_fit_alg',file_path,parameters%V_fit_alg)
        call GetLogical('use_NE',file_path,parameters%use_NE)
        call GetReal('SVD_disc',file_path,parameters%SVD_disc)
        call GetReal('rr_delta',file_path,parameters%rr_delta)
        call GetLogical('GD_use_FS',file_path,parameters%GD_use_FS)
        call GetReal('GD_reg_lambda',file_path,parameters%GD_reg_lambda)
        call GetInt('GD_max_iter',file_path,parameters%GD_max_iter)
        call GetReal('GD_alpha',file_path,parameters%GD_alpha)
        call GetReal('GD_stop_crit',file_path,parameters%GD_stop_crit)
        call GetInt('GD_MB_iter',file_path,parameters%GD_MB_iter)
        call GetIntMat('GD_MB_threshold',file_path,parameters%GD_MB_threshold)
        call GetRealMat('GD_MB_share',file_path,parameters%GD_MB_share)

        call GetLogical('gen_mod_V_reg',file_path,parameters%gen_mod_V_reg)
        call GetInt('GAF_FS',file_path,parameters%GAF_FS)
        call GetIntMat('GAF_basis',file_path,parameters%GAF_basis)
        call GetIntMat('GAF_deg',file_path,parameters%GAF_deg)
        call GetIntMat('GAF_inter_deg',file_path,parameters%GAF_inter_deg)
        ...
        !In future (low priority) compute some additional variables -> So far can keep it here but if
        !there are more of them, put this into an additional subroutine.
        !This could be written more efficiently but it doesn't matter because this
        !subroutine is called only once.
        ...
        if(parameters%rel_growth>=1.0_dp) then
            write(*,*) 'Error: growth rate of productivity of agent one must be strictly larger than that of other agents.'
            error stop
        end if
...
subroutine check_parameters(parameters,folder_name)
        ...
        !debug: check whether error_message is rewritten automatically every time...

!Also check the condition on parameters which guarantees something about steady state (see the matlab file)
