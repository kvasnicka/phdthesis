...
!Trivial function converting logical variable into integer (True
!treated as 1, false treated as zero)
function L_to_I(L)
    integer :: L_to_I
    logical, intent(in) :: L
...
!Function proj_to_m11 (minus 1, 1) projects values from interval [a,b] linearly onto
!interval [-1,1]. This is useful primarily in Chebyshev polynomial approximation.
!(z is the input).
function proj_to_m11(z,a,b)
    real(dp) :: proj_to_m11
    real(dp), intent(in) :: z,a,b

    proj_to_m11 = (2*z - a - b)/(b-a)

   !Experiment: Project the states into -0.7,0.7 interval instead (idea is that Cheb pol
   !might not be so nicely behaved towards the boundary of where they are defined).
   !It actually helps with the approximation quite a bit, especially if we're using
   !deterministic grids.
   proj_to_m11 = 0.7_dp * proj_to_m11

end function proj_to_m11
